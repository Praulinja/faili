class Cilveks:
    def __init__(self, vards, vecums, dzimums):
        self.age = vecums
        self.name = vards
        self.sex = dzimums
        self.nauda = 0
#sakuma parametri
#self norada, tieši to objektu - sevi

    def dzimsanas_diena(self):
        self.age += 1
#dzimsanas dienaa palielinas gadu skaits

    def mainit_vardu(self, jaunais_vards):
        self.name = jaunais_vards
#metode - piemīt objektiem

    def pastastit_par_sevi(self):
        if self.sex == "s":
            dz = "sieviete"
        elif self.sex == "v":
            dz = "vīrietis"
        else:
            dz = self.sex
        print("Mani sauc {}, man ir {} gadi, es esmu {}, man ir {} EUR".format(self.name, self.age, dz, self.nauda))
#procedūra - neko nedara, tikai kaut ko parāda
        
    def nopelnit(self, nopelnita_nauda):
        self.nauda += nopelnita_nauda
    
    def nomainit_dzimumu(self, jaunais_dzimums):
        self.gender = jaunais_dzimums

    def uztaisit_spamu(self, spama_mape):
        if self.sex == "s":
            sveiki_galotne = "a"
            laimests_galotne = "usi"
        if self.sex == "v":
            sveiki_galotne = "s"
            laimests_galotne = "is"
        else:
            sveiki_galotne = "i"
            laimests_galotne = "is"
        
        faila_nosaukums = spama_mape + "spams" + self.name + str(self.age) + ".txt"
        
        teksts = "Sveik{}, {}! Tu esi laimēj{} {}€!".format(sveiki_galotne, self.name, laimests_galotne, self.age*35)

        with open(faila_nosaukums, "w", encoding="utf-8") as fails:
            fails.write(teksts)

        self.nopelnit(self.age*35)
        self.pastastit_par_sevi()

persona1 = Cilveks("Delle", 45, "s")
persona1.pastastit_par_sevi()
persona1.dzimsanas_diena()
persona1.pastastit_par_sevi()
persona1.nopelnit(30)
persona1.pastastit_par_sevi()
persona1.uztaisit_spamu("spams/")

turpinat = "t"
cilveki = []
while turpinat == "t":
    vards = input("Ievadiet cilvēka vārdu!: ")
    vecums = int(input("Ievadiet vecumu!: "))
    dzimums = input("Ievadiet dzimumu (s/v)!:")
    cilveki.append( Cilveks(vards, vecums, dzimums) )
    turpinat = input("Ja vēlies pievienot vēl vienu cilvēku, nospied 't' !")

for viens in cilveki:
    viens.uztaisit_spamu("spams/")

#katram cilvekam uztaisit spamu nevis par sevi pastastit